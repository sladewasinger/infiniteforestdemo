export class PerlinNoise {
  /// <summary>
  /// Seed should be between 0 and 1
  /// </summary>
  constructor(seed) {
    this.seed = seed;
    this.permutation = [
    ...
    // shuffle permutation by seed
    const shuffle = (seed) => {
      let j, x, i;
      for (i = this.permutation.length - 1; i > 0; i--) {
        j = Math.floor((i + 1) * seed);
        x = this.permutation[i];
  ...
  noise2d(x, y) {
    ...
    return this.scale(
      this.lerp(
        ...
        this.lerp(
          u,
          this.grad(p[AB], x, y - 1, 0),
          this.grad(p[BB], x - 1, y - 1, 0)
        )
      )
    );
  }
